...
AddEventHandler('probar:permisos', function(loadWeed)
	...
	local player = vRP.getUserSource(user_id)
		if vRP.hasGroup(user_id,"Traficante de Maconha") then
...
AddEventHandler('meta:permisos', function(loadWeed)
	local user_id = vRP.getUserId(source)
	local player = vRP.getUserSource(user_id)
		if vRP.hasGroup(user_id,"Traficante de Metanfetamina") then
...
AddEventHandler('coke:permisos', function(loadWeed)
	local user_id = vRP.getUserId(source)
	local player = vRP.getUserSource(user_id)
		if vRP.hasGroup(user_id,"Traficante de Cocaina") then
