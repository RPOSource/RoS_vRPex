...
local function ch_whitelist(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.whitelist") then
    local id = vRP.prompt(player,"User Id para whitelist: ","")
    id = parseInt(id)
...
local function ch_unwhitelist(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.unwhitelist") then
    local id = vRP.prompt(player,"User Id para un-whitelist: ","")
    id = parseInt(id)
...
local function ch_addgroup(player,choice)
  ...
  if user_id ~= nil and vRP.hasPermission(user_id,"player.group.add") then
    ...
    id = parseInt(id)
    local group = vRP.prompt(player,"Gropo para add: ","")
    if group then
      vRP.addUserGroup(id,group)
      vRPclient._notify(player, group.." added pelo user "..id)
    end
  end
end
...
local function ch_removegroup(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.group.remove") then
    local id = vRP.prompt(player,"User Id: ","")
    id = parseInt(id)
    local group = vRP.prompt(player,"Gropo para remover: ","")
    if group then
      vRP.removeUserGroup(id,group)
      vRPclient._notify(player, group.." removido pelo user "..id)
    end
  end
end
...
local function ch_kick(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.kick") then
    local id = vRP.prompt(player,"User id para dar kick: ","")
    id = parseInt(id)
    local reason = vRP.prompt(player,"Razão: ","")
    local source = vRP.getUserSource(id)
    if source then
      vRP.kick(source,reason)
      vRPclient._notify(player, "kick user "..id)
    end
  end
end
...
local function ch_ban(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.ban") then
    local id = vRP.prompt(player,"User Id para banir: ","")
    id = parseInt(id)
    local reason = vRP.prompt(player,"Razão: ","")
    local source = vRP.getUserSource(id)
    if source then
      vRP.ban(source,reason)
      vRPclient._notify(player, "Banido user "..id)
    end
  end
end
...
local function ch_unban(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.unban") then
    ...
    vRPclient._notify(player, "Desbanir user "..id)
  end
end
...
local function ch_emote(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.custom_emote") then
    local content = vRP.prompt(player,"Sequencia de Animação ('dict anim optional_loops' per line): ","")
    local seq = {}
...
local function ch_sound(player,choice)
  ...
  if user_id and vRP.hasPermission(user_id,"player.custom_sound") then
    local content = vRP.prompt(player,"Som 'dict name': ","")
      local args = {}
...
local function ch_coords(player,choice)
  ...
  vRP.prompt(player,"Copie as coordenas usando Ctrl-A Ctrl-C",x..","..y..","..z)
end
...
local function ch_tptocoords(player,choice)
  local fcoords = vRP.prompt(player,"Coordenadas x,y,z:","")
  local coords = {}
...
local function ch_givemoney(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id then
    local amount = vRP.prompt(player,"Quantia:","")
...
local function ch_giveitem(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id then
    local idname = vRP.prompt(player,"Nome:","")
    idname = idname or ""
    local amount = vRP.prompt(player,"Quantidade:","")
    amount = parseInt(amount)
...
local function ch_calladmin(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id then
    local desc = vRP.prompt(player,"Descreva seu problema:","") or ""
    ...
    for k,v in pairs(players) do
      ...
      if ok then -- take the call
        if not answered then
          -- answer the call
          vRPclient._notify(player,"um admin aceitou seu ticket.")
          vRPclient._teleport(v, vRPclient.getPosition(player))
          answered = true
        else
          vRPclient._notify(v,"Ticket já foi aceito.")
        end
      end
    end
  end
