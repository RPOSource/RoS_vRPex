...
function task_mission()
            ...
            local step = {
              ...
              onenter = function(player, area)
                if vRP.tryGetInventoryItem(user_id, "repairkit", 1, false) then
                  ...
                  vRPclient.notify(player, "~r~Você precisa de um kit de reparo!")
                end
              end,
  ...
  -- Lenhador
  for k, v in pairs(cfg.lenhador) do -- each repair perm def
    -- add missions to users
    local users = vRP.getUsersByPermission(k)
    for l, w in pairs(users) do
      local user_id = w
      ...
      if not vRP.hasMission(player) then
        if math.random(1, v.chance) == 1 then -- chance check
          -- build mission
          local mdata = {}
          mdata.name = lang.lenhador.title()
            ...
            onenter = function(player, area)
              for idname, amount in pairs(delivery_items) do
                if amount > 0 then -- check if not done
                  ...
                  vRPclient.notify(player, "~r~Algo está faltando!")
                end
              end
            end,
            position = v.positions[math.random(1, #v.positions)]
          ...
          for idname, amount in pairs(delivery_items) do
            local name = vRP.getItemName(idname)
            step.text = step.text .. lang.lenhador.item({name, amount}) .. "<br />"
          end
              ...
              for idname, amount in pairs(delivery_items) do
                if amount > 0 then -- check if not done
                  if vRP.tryGetInventoryItem(user_id, idname, amount, true) then
                    local reward = v.items[idname][3] * amount
                    if
                      k == "mission.delivery.pot" or k == "mission.delivery.pot2" or k == "mission.delivery.pot3" or
                        k == "mission.delivery.pot4"
                     then
    ...
    for l, w in pairs(users) do
      ...
      if not vRP.hasMission(player) then
        if math.random(1, v.chance) == 1 then -- chance check -- chance check
          -- build mission
          local mdata = {}
          mdata.name = lang.bankdriver.title()
          mdata.steps = {}

          -- build steps
          for i = 1, v.steps do
            local step = {
              text = lang.bankdriver.title() .. "<br />" .. lang.reward({v.reward}),
              onenter = function(player, area)
